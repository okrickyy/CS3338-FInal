...
\hline
2.0 &
Expanded SRS with added assumptions and dependencies, user
characteristics, refined hardware and software interface requirements,
and additional legal/ethical and glossary entries for
\textit{AstroStreet AR}. &
December 8, 2025 \\
\hline
3.0 &
Final version of the SRS including non-functional requirements
(performance, usability, reliability and safety) and explicit future
requirements and out-of-scope features for \textit{AstroStreet AR}. &
December 11, 2025 \\
\hline
\end{tabularx}
\end{center}
...
The requirements in this SRS are written under the following assumptions
and dependencies:

\begin{itemize}
    \item \textbf{A-1:} Players have access to a modern smartphone with a
    functioning rear-facing camera, motion sensors, and sufficient battery
    charge for short play sessions.
    \item \textbf{A-2:} The device supports a compatible augmented reality
    framework (such as ARCore or ARKit) through the chosen game engine.
    \item \textbf{A-3:} Players are familiar with common mobile game
    interactions, such as tapping buttons and navigating simple menus.
    \item \textbf{A-4:} The game will be distributed through a standard
    mobile app store (e.g., Google Play or Apple App Store), and thus must
    comply with the store's basic technical and content guidelines.
    \item \textbf{A-5:} The initial project scope targets single-player,
    offline gameplay; online features such as cloud leaderboards are
    considered future enhancements.
\end{itemize}

These assumptions help bound the problem space and clarify conditions
under which the stated requirements are expected to hold.

\subsection{User Characteristics}

\textit{AstroStreet AR} is designed for a broad audience of casual
players rather than expert gamers.

\begin{itemize}
    \item \textbf{UC-1:} Users may have varying levels of gaming
    experience, so the game should be understandable without requiring
    prior knowledge of first-person shooters or AR games.
    \item \textbf{UC-2:} Users may play in short sessions (e.g., a few
    minutes at a time), so the game should allow quick entry into
    gameplay and not require a lengthy tutorial.
    \item \textbf{UC-3:} Users may play in outdoor or semi-public spaces,
    so on-screen text and UI elements should be readable in typical
    lighting conditions and not rely on very small fonts.
    \item \textbf{UC-4:} Some users may have limited physical mobility
    or may prefer to rotate in place rather than walk around; the game
    should remain playable without requiring extensive walking.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{HW-5:} The device should have sufficient processing
    power and memory to run a basic AR application without severe
    frame rate drops during simple scenes (for example, at least a
    mid-range smartphone released within the last several years).
    \item \textbf{HW-6:} The camera should be capable of capturing
    color images at a resolution suitable for AR tracking under normal
    lighting conditions.
...
\begin{itemize}
    ...
    documented APIs.
    \item \textbf{SW-5:} The system shall request and obtain operating
    system permissions required for AR functionality (such as camera
    access and motion sensor access) before starting an AR gameplay
    session.
    \item \textbf{SW-6:} If the required AR framework or permissions
    are not available or are denied by the user, the system shall
    display an explanatory message and prevent AR gameplay from
    starting.
...
To make \textit{AstroStreet AR} accessible to a wider range of players,
the system shall consider basic accessibility and inclusivity guidelines.

\begin{itemize}
    \item \textbf{ACC-1:} The system shall avoid relying solely on color
    to convey critical gameplay information and shall ensure that key UI
    elements (such as health and score) have sufficient contrast against
    the camera background.
    \item \textbf{ACC-2:} The system shall avoid rapidly flashing
    visual effects that could pose a risk to users with photosensitive
    conditions.
    \item \textbf{ACC-3:} The system should provide an option to reduce
    or disable non-essential visual effects (such as particle bursts)
    in order to improve clarity on small screens.
    \item \textbf{ACC-4:} Text used in safety warnings and instructions
    shall be displayed in a readable font size appropriate for mobile
    devices.
\end{itemize}
...
This section describes qualities of \textit{AstroStreet AR} that are not tied
to a single function but affect the overall experience, including performance,
usability, reliability, and safety.

\subsection{Performance Requirements}

\begin{itemize}
    \item \textbf{NFR-P1:} Under typical conditions (moderate number of enemies
    and projectiles), the system should maintain an effective frame rate of
    at least 30 frames per second on supported devices.
    \item \textbf{NFR-P2:} The game should load from the Main Menu into the
    AR gameplay state within a reasonable time (e.g., a few seconds), assuming
    the AR framework is available and permissions have been granted.
    \item \textbf{NFR-P3:} The system should avoid creating unbounded numbers
    of enemies or projectiles; resource usage should scale within limits
    defined by the design (e.g., a maximum number of active enemies).
\end{itemize}

\subsection{Usability Requirements}

\begin{itemize}
    \item \textbf{NFR-U1:} A new user should be able to start a game from the
    Main Menu and understand the basic objective (shooting virtual enemies
    overlaid on the real world) within their first session, without reading
    external documentation.
    \item \textbf{NFR-U2:} Text for key UI elements (score, health, basic
    instructions) shall be of a readable size for typical smartphone screens
    and remain legible against the camera background.
    \item \textbf{NFR-U3:} Navigation between main screens (Main Menu,
    Gameplay, Pause, Game Over, High Scores, Settings) shall be achievable
    with simple tap interactions, without requiring multi-finger gestures.
\end{itemize}

\subsection{Reliability and Safety Requirements}

\begin{itemize}
    \item \textbf{NFR-R1:} The system shall handle loss of AR tracking or
    camera access gracefully by pausing or limiting gameplay and notifying
    the user, rather than crashing.
    \item \textbf{NFR-R2:} In the event of an unexpected interruption
    (such as the app being backgrounded), the system should avoid corrupting
    stored data such as high scores or settings.
    \item \textbf{NFR-S1:} The system shall display at least a brief safety
    reminder encouraging users to be aware of their surroundings before
    starting AR gameplay.
    \item \textbf{NFR-S2:} The system shall not encourage dangerous behavior,
    such as running while playing or using the application in restricted or
    hazardous areas.
\end{itemize}

% --- 6. Future Requirements and Out-of-Scope Features
\section{Future Requirements and Out-of-Scope Features}

The initial version of \textit{AstroStreet AR} implemented for this course
focuses on a single-player, offline AR shooting experience with local
high scores. The following features are explicitly considered future
requirements and are out of scope for the current implementation:

\begin{itemize}
    \item \textbf{FR-F1: Online Leaderboards.} Synchronizing high scores and
    player statistics with a remote server so that players can compare scores
    globally or with friends.
    \item \textbf{FR-F2: Additional Game Modes.} Alternate modes such as timed
    challenges, endless survival, or cooperative play that require new game
    rules and possibly networked communication.
    \item \textbf{FR-F3: Advanced Enemy Behaviors.} More complex enemy patterns
    (e.g., coordinated attacks, boss encounters) that extend beyond the basic
    movement and spawning rules described in the current requirements.
    \item \textbf{FR-F4: Expanded Accessibility Options.} Additional settings
    such as configurable HUD sizes, colorblind-friendly palettes, or haptic
    feedback toggles beyond those included in the initial scope.
    \item \textbf{FR-F5: Cross-Platform Features.} Support for non-mobile AR
    platforms (such as dedicated AR headsets) or cross-device synchronization.
\end{itemize}

Documenting these future requirements clarifies the boundaries of the current
project and provides guidance for potential continuation or future teams who
may wish to extend \textit{AstroStreet AR}.
...
\hline
Pose &
The position and orientation of the device in 3D space, as reported
by the AR framework (e.g., where the phone is and which direction it
is facing). \\
\hline
Anchor &
A fixed reference point in the real world that the AR framework uses
to place and maintain virtual objects at stable positions. \\
\hline
Frame Rate &
The number of times per second the game updates and redraws the screen,
typically measured in frames per second (FPS). Higher frame rates
generally result in smoother motion. \\
\hline
Field of View (FOV) &
The extent of the observable world that is visible through the camera
and displayed on the screen at any given moment. \\
\hline
Accessibility &
Design practices that make the game usable by people with a wide range
of abilities, including considerations for vision, motion, and other
differences. \\
\hline
\end{tabularx}
\end{center}
